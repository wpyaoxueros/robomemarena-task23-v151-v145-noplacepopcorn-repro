# RoboMemArena Task23 v151 Reproduction

这是 Task23 的一个不可变评测版本仓库。每个版本一个仓库；不覆盖之前版本。

## 版本结论

- 继承版本：Task23 v145 commit `12c589b`，具体复用 seed105 这条达到前两 stage 的运行方法。
- 唯一行为修改：从 Task23 end-pose target 和 consecutive-hold 配置中删除 `place popcorn`。
